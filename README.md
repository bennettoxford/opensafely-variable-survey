...
5. Writes the aggregated results to `data/ehrql_variables.json` and captures the
...
- `data/ehrql_variables.json` / `ehrql_qm_dump.json` – outputs generated by
  the collector.
...
The `data/ehrql_variables.json` file is currently manually copy/pasted into [this repository](https://github.com/bennettoxford/rsi-prototypes/)
and made available via a tool deployed [here](https://prototypes.bennettoxford.org/ehrql-variable-explorer).
...
We populate the [codelist quality report](https://prototypes.bennettoxford.org/codelist-audit/) with the `codelist_audit.json` file generated as follows.
